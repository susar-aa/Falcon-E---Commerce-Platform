...
// SQL query to fetch all subcategories for CategoryID = 1 (Stationeries)
$sql_subcategories = "SELECT SubCategoryID, SubCategoryName FROM SubCategory WHERE CategoryID = 1 AND AvailabilityStatus = 'Available' ORDER BY SubCategoryName";
$result_subcategories = $conn->query($sql_subcategories);

// Fetch all items and their price range for stationery category
$sql_all_items = "
    SELECT
        i.ItemID,
        i.Name,
    ...
    WHERE i.AvailabilityStatus = 'Available' AND i.CategoryID = 1
    GROUP BY i.ItemID
...
<head>
    ...
    <title>Stationeries - Falcon Stationery PVT LTD</title>
...
<body class="bg-gray-100 flex flex-col h-screen overflow-hidden">
    <header class="bg-white shadow-md flex-shrink-0">
        <div class="container mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center space-x-6">
                ...
                <a href="stationeries.php" class="text-blue-600 font-semibold border-b-2 border-blue-600 transition duration-300">Stationeries</a>
    ...
    <div class="flex-1 flex flex-col md:flex-row overflow-hidden md:py-8">
        ...
        <main class="md:w-3/4 lg:w-4/5 p-4 md:p-6 overflow-y-auto md:h-full" id="main-content-area">
            ...
            <div class="mb-6">
                <div class="relative">
                    <input type="text" id="search-input" placeholder="Search for stationeries..." class="w-full p-3 pl-10 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <h1 class="text-2xl md:text-4xl font-bold text-gray-800 mb-4 md:mb-6 text-center md:text-left">Our Stationeries Collection</h1>
            <div id="product-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                ...
                <?php
                    ...
                    echo "<p class='col-span-full text-center text-gray-500 text-lg py-12'>No stationery products found at the moment. Please check back later!</p>";
                }
                ?>
            </div>
    ...
    function saveScrollAndNavigate() {
        ...
        if (mainContent) {
            sessionStorage.setItem('stationeriesScrollPos', mainContent.scrollTop);
        }
    }

    document.addEventListener('DOMContentLoaded', () => {
        const mainContent = document.getElementById('main-content-area');
        const savedScrollPos = sessionStorage.getItem('stationeriesScrollPos');
